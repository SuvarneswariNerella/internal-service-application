...
export default function TemplateLibraryView({ onUseTemplate }: { onUseTemplate?: (template: DesignTemplate) => void }) {
  ...
  const filteredTemplates = savedTemplates.filter(t => {
    if (activeTemplateType === "ALL") return true;
    if (activeTemplateType === "PURCHASE_ORDERS") return t.type === "PURCHASE_ORDER" || t.type === "PURCHASE_ORDERS";
    return t.type === activeTemplateType;
  });
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-6 md:p-8 space-y-12 bg-gray-50/30">
          ...
          {Array.from(new Set(savedTemplates.map(t => t.workspaceId || 'system'))).map(wsId => {
            const wsTemplates = savedTemplates.filter(t => (t.workspaceId || 'system') === wsId);
            
            const activeWsTemplates = wsTemplates.filter(t => {
              if (activeTemplateType === "ALL") return true;
              if (activeTemplateType === "PURCHASE_ORDERS") return t.type === "PURCHASE_ORDER" || t.type === "PURCHASE_ORDERS";
              return t.type === activeTemplateType;
            });

            if (activeWsTemplates.length === 0) return null;

            const wsInfo = workspaces.find(w => w.id === wsId);

            return (
              <div key={wsId} className="space-y-8">
                {globalWorkspaceId === "all" && (
                  wsInfo ? (
                    <div className="flex items-center gap-4 pb-4 border-b border-gray-200">
                      {wsInfo.logoUrl ? (
                        <img src={wsInfo.logoUrl} className="w-10 h-10 rounded-lg object-contain border border-gray-200 shadow-sm bg-white" />
                      ) : (
                        <div className="w-10 h-10 rounded-lg bg-white flex items-center justify-center font-black text-gray-500 border border-gray-200 shadow-sm">
                          {wsInfo.displayName.substring(0, 2).toUpperCase()}
                        </div>
                      )}
                      <div>
                        <h2 className="text-[16px] font-black text-gray-900 flex items-center gap-2">
                          {wsInfo.displayName} 
                          {wsInfo.workspaceCode && <span className="text-[10px] font-bold text-gray-500 bg-white px-1.5 py-0.5 rounded border border-gray-200 shadow-sm">{wsInfo.workspaceCode}</span>}
                        </h2>
                        <p className="text-[12px] text-gray-500 font-medium">
                          {wsInfo.gstin ? `GSTIN: ${wsInfo.gstin} • ` : ''}{wsTemplates.length} Design Blueprints
                        </p>
                      </div>
                    </div>
                  ) : (
                    <div className="flex items-center gap-4 pb-4 border-b border-gray-200">
                      <div className="w-10 h-10 rounded-lg bg-[#5438FF]/10 flex items-center justify-center border border-[#5438FF]/20">
                        <LayoutTemplate className="w-5 h-5 text-[#5438FF]" />
                      </div>
                      <div>
                        <h2 className="text-[16px] font-black text-gray-900">System Templates</h2>
                        <p className="text-[12px] text-gray-500 font-medium">
                          {wsTemplates.length} Design Blueprints
                        </p>
                      </div>
                    </div>
                  )
                ...
                <div className={`space-y-10 ${globalWorkspaceId === "all" ? "pl-2" : ""}`}>
                  {GROUP_CONFIGS.map(group => {
                    if (activeTemplateType !== "ALL" && activeTemplateType !== group.type) return null;
                    
                    const groupTemplates = wsTemplates.filter(t => t.type === group.type || (group.type === "PURCHASE_ORDERS" && t.type === "PURCHASE_ORDER"));
          ...
          {filteredTemplates.length === 0 && (
             <div className="text-center py-20">
               ...
               <h3 className="text-sm font-bold text-gray-900 mb-1">
                 {globalWorkspaceId === "all" ? "No templates found in all workspaces" : "No templates found"}
               </h3>
               <p className="text-xs text-gray-500">
                 {activeTemplateType !== "ALL" ? `No templates available for the selected category.` : `Create a custom template to see it here.`}
               </p>
             </div>
          )}
